...
def export_monthly_report_pdf(user_id, month=None, year=None, filename=None):
    ...
    styles.add(
        ParagraphStyle(
            name="SectionHeader",
            fontSize=14,
            textColor=colors.HexColor("#004B8D"),
            spaceAfter=10,
            spaceBefore=15,
            leading=16,
        )
    ...
    styles.add(
        ParagraphStyle(name="Footer", fontSize=9, textColor=colors.grey, alignment=1)
    )
    ...
    elements.append(
        HRFlowable(width="100%", color=colors.HexColor("#004B8D"), thickness=1)
    )
    ...
    savings_color = (
        colors.HexColor("#009E60")
        if result["savings"] >= 0
        else colors.HexColor("#B22222")
    )
    ...
    elements.append(
        Paragraph("📂 <b>Category-wise Expenses</b>", styles["SectionHeader"])
    )
    ...
    if cat_rows:
        ...
        elements.append(
            Paragraph(
                "<i>No category expenses found for this month.</i>", styles["Normal"]
            )
        )
    ...
    elements.append(
        HRFlowable(width="100%", color=colors.HexColor("#004B8D"), thickness=0.8)
    )
    elements.append(Spacer(1, 0.1 * inch))
